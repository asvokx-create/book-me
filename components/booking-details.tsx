...
export default function BookingDetails({ bookingId, expectedRole }: { bookingId: string; expectedRole: "customer" | "provider" }) {
  ...
  const canComplete = booking.viewerRole === "provider" && booking.status === "confirmed" && booking.paymentStatus === "paid";
  ...
  return <>
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
      ...
      <section className="overflow-hidden rounded-[2rem] border border-[#183126]/10 bg-white shadow-[0_8px_30px_rgba(24,49,38,.05)]">
        ...
        {booking.paymentStatus === "paid" && <div className="border-t border-[#183126]/10 bg-[#f7f8f3] px-6 py-5 sm:px-8"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.13em] text-[#718078]">Payment progress</p><p className="mt-2 font-bold">{releaseCopy.label}</p><p className="mt-1 max-w-2xl text-sm leading-6 text-[#61736a]">{releaseCopy.detail}</p>{booking.paymentRelease.freezeReason && <p className="mt-2 text-xs font-semibold text-[#934927]">Admin hold: {booking.paymentRelease.freezeReason}</p>}</div><div className="min-w-56 rounded-2xl bg-white p-4 text-sm"><div className="flex justify-between gap-4 text-[#61736a]"><span>Service</span><span>${booking.price.toFixed(2)}</span></div><div className="mt-2 flex justify-between gap-4 text-[#61736a]"><span>Customer service fee</span><span>${booking.customerServiceFee.toFixed(2)}</span></div>{booking.viewerRole === "customer" ? <div className="mt-2 flex justify-between gap-4 border-t border-[#183126]/10 pt-2 font-bold"><span>Total paid</span><span>${customerPaymentTotal.toFixed(2)}</span></div> : <><div className="mt-2 flex justify-between gap-4 text-[#61736a]"><span>Provider booking fee</span><span>−${booking.paymentRelease.platformFee.toFixed(2)}</span></div><div className="mt-2 flex justify-between gap-4 border-t border-[#183126]/10 pt-2 font-bold"><span>Your share</span><span>${booking.paymentRelease.providerPayout.toFixed(2)}</span></div></>}</div></div></div>}
      </section>
      ...
      <aside className="space-y-5">
        <div className="rounded-[2rem] border border-[#183126]/10 bg-white p-6">
          ...
          <div className="mt-5 grid gap-3">
            ...
            {booking.viewerRole === "customer" && booking.paymentRelease.status === "awaiting_customer" && <Link href={`/disputes?bookingId=${booking.id}`} className="rounded-full bg-[#9b4e3a] px-5 py-3 text-center text-sm font-bold text-white transition hover:bg-[#7f3d2d]">Report a problem</Link>}
            ...
            {booking.viewerRole !== "worker" && booking.status !== "requested" && !(booking.viewerRole === "customer" && booking.paymentRelease.status === "awaiting_customer") && <Link href={`/disputes?bookingId=${booking.id}`} className="rounded-full border border-[#183126]/15 px-5 py-3 text-center text-sm font-bold transition hover:bg-[#fff3b0]">Open a dispute</Link>}
          </div>
