...
export default function LocationManager() {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/providers/locations", { method: editingId ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ locationId: editingId, companyId, name, location, serviceRadiusMiles: radius, workerIds, serviceAreas: serviceAreas.split(/[\n;]+/).map((item) => item.trim()).filter(Boolean) }) });
    ...
    <div className="mt-7 grid gap-6 xl:grid-cols-[.85fr_1.15fr]">
      <section className="rounded-[2rem] border border-[#183126]/10 bg-white p-6"><h2 className="text-xl font-bold">{editing ? `Edit ${editing.name}` : "Add a service location"}</h2><p className="mt-1 text-sm text-[#738179]">Use a clear internal name such as “Issaquah” or “Downtown branch.”</p>
        {!canAdd && !editing ? <div className="mt-5 rounded-2xl bg-[#fff7cb] p-5"><p className="font-bold">Starter includes one location</p><p className="mt-2 text-sm leading-6 text-[#746b40]">Upgrade to Pro to add branches and additional service areas.</p><Link href="/provider/dashboard/billing" className="mt-4 inline-flex rounded-full bg-[#183126] px-4 py-2 text-xs font-bold text-white">View Pro</Link></div> : <form onSubmit={save} className="mt-5 space-y-4">
          ...
          <label className="block text-sm font-bold">Additional cities or ZIP codes<textarea value={serviceAreas} onChange={(event) => setServiceAreas(event.target.value)} rows={4} placeholder={"98027\nSammamish, WA"} className="mt-2 w-full resize-y rounded-2xl border border-[#183126]/15 bg-[#faf9f5] px-4 py-3 font-normal outline-none" /><span className="mt-2 block text-xs font-normal leading-5 text-[#738179]">Optional. Enter one city, state or ZIP code per line. Customers match when they are inside your radius or one of these explicit areas.</span></label>
...
function LocationCards({ locations, onEdit, onRemove }: { locations: Location[]; onEdit: ((item: Location) => void) | null; onRemove: ((item: Location) => void) | null }) {
  return <div className="mt-5 space-y-3">{locations.length ? locations.map((item) => <article key={item.id} className="rounded-2xl bg-[#f5f5ef] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><p className="font-bold">{item.name}</p>{item.isPrimary && <span className="rounded-full bg-[#e3eee0] px-2 py-1 text-[10px] font-bold uppercase text-[#39704a]">Primary</span>}</div><p className="mt-1 text-sm text-[#687970]">{item.location} · Within {item.serviceRadiusMiles} mi</p>{item.serviceAreas.length > 0 && <p className="mt-2 max-w-xl text-xs leading-5 text-[#5f7468]">Also serving: {item.serviceAreas.join(" · ")}</p>}<p className="mt-2 text-xs text-[#7b8982]">{item.companyName} · {item.listingCount} {item.listingCount === 1 ? "listing" : "listings"} · {item.workerIds.length} {item.workerIds.length === 1 ? "worker" : "workers"}</p></div>{onEdit && <div className="flex gap-2"><button onClick={() => onEdit(item)} className="rounded-full border border-[#183126]/15 bg-white px-3 py-2 text-xs font-bold">Edit</button>{!item.isPrimary && onRemove && <button onClick={() => onRemove(item)} className="rounded-full px-3 py-2 text-xs font-bold text-[#914e3a]">Remove</button>}</div>}</div></article>) : <p className="rounded-2xl bg-[#f5f5ef] p-5 text-sm text-[#738179]">No service locations are assigned yet.</p>}</div>;
}
