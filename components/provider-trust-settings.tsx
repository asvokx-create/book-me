...
export default function ProviderTrustSettings({ initial }: { initial: TrustSettings }) {
  ...
  async function runAutomaticVerification() {
    ...
    const data = response ? await response.json() as { error?: string; result?: { overall: "passed" | "needs_changes"; score: number; checkedAt: string; checks: Array<{ key: string; label: string; passed: boolean; detail: string }> } } : null;
    setBusy(false);
    ...
    setVerification({ status: data.result.overall, score: data.result.score,
      summary: data.result.overall === "passed" ? "Automated account, phone-detail, Stripe identity, and business-profile checks passed." : data.result.checks.filter((check) => !check.passed).map((check) => check.detail).join(" "),
      checkedAt: data.result.checkedAt });
