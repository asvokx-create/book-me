  ...
  return <main className="min-h-screen overflow-x-clip bg-[#f4f4ef] text-[#183126]">
    ...
    <div className="mx-auto min-w-0 max-w-6xl px-4 py-7 sm:px-5 sm:py-10">
      ...
      <div className="mt-7 space-y-4">
        {tab === "support" ? support.map((item) => <article key={item.id} className="min-w-0 rounded-[1.7rem] bg-white p-5 sm:p-6"><div className="flex flex-col justify-between gap-4 sm:flex-row"><div className="min-w-0"><span className="rounded-full bg-[#fff1bf] px-3 py-1 text-xs font-bold capitalize">{item.status}</span><h2 className="mt-3 text-xl font-bold">{item.subject}</h2><p className="mt-1 text-sm text-[#718078]">{item.user_name} · {item.user_email} · {new Date(item.created_at).toLocaleString()}</p></div><div className="admin-action-row flex w-full gap-2 sm:w-auto"><button disabled={busy === item.id} onClick={() => void update("support", item.id, "reviewing")} className="rounded-full border px-4 py-2 text-sm font-bold hover:bg-[#eee25a]">Reply / review</button><button disabled={busy === item.id} onClick={() => void update("support", item.id, "resolved")} className="rounded-full bg-[#183126] px-4 py-2 text-sm font-bold text-white">Resolve</button></div></div><p className="mt-5 whitespace-pre-wrap rounded-2xl bg-[#f7f7f2] p-5 text-sm leading-6">{item.message}</p>{item.admin_reply && <p className="mt-3 rounded-2xl bg-[#e7f1e3] p-4 text-sm"><strong>Admin reply:</strong> {item.admin_reply}</p>}</article>) : <>
