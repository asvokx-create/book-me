...
import { ALL_DAY_END_TIME, ALL_DAY_START_TIME, isAllDayAvailability } from "@/lib/availability-hours";

type AvailabilitySlot = { weekday: number; startTime: string; endTime: string };
type DayState = AvailabilitySlot & { enabled: boolean; allDay: boolean };
...
export default function AvailabilityEditor({ serviceId, serviceTitle, initialSlots, onSaved }: { serviceId: string; serviceTitle: string; initialSlots: AvailabilitySlot[]; onSaved: (slots: AvailabilitySlot[]) => void }) {
  ...
  function setAllDay(weekday: number, allDay: boolean) {
    updateDay(weekday, allDay
      ? { allDay: true, startTime: ALL_DAY_START_TIME, endTime: ALL_DAY_END_TIME }
