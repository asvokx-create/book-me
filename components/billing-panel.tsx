...
export default function BillingPanel({ plan }: { plan: ProviderPlan }) {
  ...
  useEffect(() => {
    const noticeTimer = window.setTimeout(() => {
      ...
      if (params.get("stripe") === "subscription-success") setNotice("Stripe received your subscription. Your plan or free trial will update after the secure webhook confirms it.");
  ...
  async function changeSubscription(action: "cancel" | "resume") {
    if (action === "cancel" && !window.confirm("Cancel Pro at the end of your current billing period? You will keep Pro access until then and will not be charged again.")) return;
    setWorking(`subscription-${action}`); setError(""); setNotice("");
    const response = await fetch("/api/stripe/subscriptions/manage", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action }),
    }).catch(() => null);
    const result = response ? await response.json() as { cancelAtPeriodEnd?: boolean; subscriptionStatus?: string; currentPeriodEnd?: string | null; error?: string } : null;
    setWorking("");
    if (!response?.ok || result?.cancelAtPeriodEnd === undefined) {
      setError(result?.error ?? "We could not update your subscription. Please try again.");
      return;
    }
    setStatus((currentStatus) => currentStatus ? {
      ...currentStatus,
      cancelAtPeriodEnd: result.cancelAtPeriodEnd!,
      subscriptionStatus: result.subscriptionStatus ?? currentStatus.subscriptionStatus,
      currentPeriodEnd: result.currentPeriodEnd ?? currentStatus.currentPeriodEnd,
    } : currentStatus);
    setNotice(action === "cancel"
      ? `Your Pro subscription is canceled for renewal. You will keep access through ${formatBillingDate(result.currentPeriodEnd)}.`
      : "Your Pro subscription will continue and renew normally.");
  }
  ...
  const monthlyTotal = current.monthlyPrice + (plan === "pro" ? (status?.extraTeamSeats ?? 0) * 0.5 : 0);
  const trialEndLabel = status?.currentPeriodEnd
    ? new Intl.DateTimeFormat("en-US", { dateStyle: "long" }).format(new Date(status.currentPeriodEnd))
    : "the date shown in Stripe";

  const cancellationScheduled = Boolean(hasSubscription && status?.cancelAtPeriodEnd);
  ...
  return <div>
    ...
    <div className="mt-7 grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
      <section className="rounded-[2rem] bg-[#183126] p-7 text-white"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#b4c5bc]">Current plan</p><h2 className="mt-2 text-3xl font-bold">{current.name}</h2></div><span className="rounded-full bg-[#eee25a] px-4 py-2 text-xs font-bold text-[#183126]">{isOwner ? "Private" : cancellationScheduled ? "Cancellation scheduled" : isTrialing ? "Trial active" : "Active"}</span></div><p className="mt-7 text-4xl font-bold">${isTrialing ? "0.00" : monthlyTotal.toFixed(2)}<span className="text-sm font-normal text-[#b7c7be]">{isTrialing ? " during trial" : " / month"}</span></p>{isTrialing && <p className="mt-2 text-sm font-semibold text-[#d8e5dd]">{cancellationScheduled ? `Your trial ends ${trialEndLabel}. You will not be charged.` : `Renews at $${monthlyTotal.toFixed(2)}/month on ${trialEndLabel} unless canceled.`}</p>}{cancellationScheduled && !isTrialing && <p className="mt-2 text-sm font-semibold text-[#d8e5dd]">Pro access continues through {trialEndLabel}. Your subscription will not renew.</p>}{plan === "pro" && Boolean(status?.extraTeamSeats) && <p className="mt-1 text-xs text-[#b7c7be]">$9.99 Pro + ${((status?.extraTeamSeats ?? 0) * 0.5).toFixed(2)} employee seats</p>}<div className="mt-6 grid gap-3 sm:grid-cols-2"><BillingFact label="Booking fee" value={`${current.bookingFeePercent}%`} /><BillingFact label="Services" value={current.serviceLimit === null ? "Unlimited" : `Up to ${current.serviceLimit}`} /><BillingFact label="Team seats" value={current.teamSeatLimit === null ? "Unlimited" : String(current.teamSeatLimit + (plan === "pro" ? status?.extraTeamSeats ?? 0 : 0))} /><BillingFact label="Features" value={isOwner ? "All unlocked" : current.advancedAnalytics ? "Advanced" : "Essentials"} /></div>{!isOwner && !hasSubscription && status?.trialEligible && <p className="mt-6 rounded-2xl bg-white/10 p-4 text-sm leading-6 text-[#d8e5dd]"><strong className="text-white">Try Pro free for 30 days.</strong> A card is required. You will pay $9.99 per month after the trial unless you cancel before it ends.</p>}<div className="mt-7 flex flex-wrap gap-2">{isOwner ? <span className="rounded-full bg-white/10 px-5 py-3 text-sm font-bold">Only your owner account can use this plan</span> : hasSubscription ? <><button disabled={Boolean(working)} onClick={() => void openStripe("/api/stripe/customer-portal")} className="rounded-full bg-white px-5 py-3 text-sm font-bold text-[#183126] transition hover:bg-[#eee25a] disabled:opacity-50">Manage billing</button><button disabled={Boolean(working)} onClick={() => void changeSubscription(cancellationScheduled ? "resume" : "cancel")} className={`rounded-full border px-5 py-3 text-sm font-bold transition disabled:opacity-50 ${cancellationScheduled ? "border-[#eee25a]/60 text-[#fff6a5] hover:bg-white/10" : "border-white/25 text-white hover:bg-white/10"}`}>{working.startsWith("subscription-") ? "Updating…" : cancellationScheduled ? "Keep Pro plan" : "Cancel subscription"}</button></> : <button disabled={Boolean(working) || !status?.configured} onClick={() => void openStripe("/api/stripe/subscriptions/checkout", { plan: "pro" })} className="rounded-full bg-[#eee25a] px-5 py-3 text-sm font-bold text-[#183126] hover:bg-[#f5ea6b] disabled:opacity-50">{status?.trialEligible ? "Start 30-day free trial" : "Choose Pro for $9.99"}</button>}<Link href="/pricing" className="rounded-full border border-white/20 px-5 py-3 text-sm font-bold hover:bg-white/10">Compare plans</Link></div></section>
    ...
    {plan === "pro" && hasSubscription && <TeamSeatAddon currentExtraSeats={status?.extraTeamSeats ?? 0} seatDraft={seatDraft} working={working === "team-seats"} trialing={isTrialing} onChange={setSeatDraft} onSave={() => void updateTeamSeats()} />}
...
function formatBillingDate(value: string | null | undefined) { return value ? new Intl.DateTimeFormat("en-US", { dateStyle: "long" }).format(new Date(value)) : "the end of the current billing period"; }
