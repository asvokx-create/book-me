...
export default function AiAssistant() {
  ...
  useEffect(() => {
    fetch("/api/assistant", { cache: "no-store" })
      .then(async (response) => response.ok ? response.json() as Promise<{ eligible?: boolean; configured?: boolean }> : null)
      .then((data) => {
  ...
  return (
    <div className="fixed bottom-3 right-3 z-[80] text-[#183126] sm:bottom-5 sm:right-5">
      {open && (
        <section className="mb-3 flex h-[min(520px,calc(100dvh-6.5rem))] w-[min(380px,calc(100vw-1.5rem))] flex-col overflow-hidden rounded-[2rem] border border-[#183126]/10 bg-white shadow-2xl">
          ...
          <div className="flex-1 space-y-3 overflow-y-auto bg-[#fafaf6] p-4">
            ...
            {busy && <p className="w-fit rounded-2xl bg-white px-4 py-3 text-sm text-[#718078]">Thinking…</p>}
            <div ref={endRef} />
          </div>
      ...
      <button onClick={() => setOpen((value) => !value)} className="ml-auto flex items-center gap-2 rounded-full bg-[#183126] px-5 py-3 font-bold text-white shadow-xl transition hover:bg-[#315846]"><span className="text-[#eee25a]">✦</span>{open ? "Close" : "AI help"}</button>
    </div>
  );
}
