...
export default function TeamManager() {
  ...
  useEffect(() => { fetch("/api/providers/team", { cache: "no-store" }).then(async (response) => { const data = await response.json() as { members?: Member[]; plan?: ProviderPlan; seatLimit?: number | null; extraTeamSeats?: number; error?: string }; if (!response.ok) throw new Error(data.error); setMembers(data.members ?? []); setPlan(data.plan ?? "starter"); setSeatLimit(data.seatLimit === undefined ? 1 : data.seatLimit); setExtraTeamSeats(data.extraTeamSeats ?? 0); }).catch((reason: Error) => setError(reason.message)).finally(() => setLoaded(true)); }, []);
  ...
  return <div><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm font-semibold text-[#687a70]">Company access</p><h1 className="mt-1 text-3xl font-bold tracking-[-.04em] sm:text-4xl">Team</h1><p className="mt-2 text-sm text-[#687a70]">Add workers and decide who can help manage the company.</p></div><div className="flex flex-wrap items-center gap-2"><span className="w-fit rounded-full bg-[#eee25a] px-4 py-2 text-xs font-bold">{PLAN_ENTITLEMENTS[plan].name} · {seatLimit === null ? "Unlimited seats" : `${activeWorkers + 1}/${seatLimit} seats`}</span>{plan === "pro" && extraTeamSeats > 0 && <span className="rounded-full bg-[#e5f1e5] px-4 py-2 text-xs font-bold text-[#34704a]">{extraTeamSeats} paid extra</span>}</div></div>
