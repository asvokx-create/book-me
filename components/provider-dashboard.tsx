    ...
    <main className="min-h-screen scroll-smooth bg-[#f4f4ef] text-[#183126]">
      <header className="relative z-50 border-b border-[#183126]/10 bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-3 py-3 sm:px-8 sm:py-4">
          <Link href="/" className="flex min-w-0 items-center gap-2 text-lg font-bold tracking-tight sm:text-xl"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[#183126] text-sm text-[#eee25a]">B</span><span className="hidden min-[430px]:inline">BubsBookings</span><span className="min-[430px]:hidden">Bubs</span> <span className="hidden rounded-full bg-[#e8f0e5] px-2.5 py-1 text-[10px] uppercase tracking-wider text-[#55705e] sm:inline">Provider</span></Link>
          <div className="flex min-w-0 items-center gap-1 sm:gap-3">{provider?.isAdmin && <Link href="/admin" aria-label="Admin" className="rounded-full bg-[#eee25a] px-3 py-2.5 text-xs font-bold transition hover:bg-[#f5ea6b] sm:px-4 sm:text-sm"><span className="min-[460px]:hidden">A</span><span className="hidden min-[460px]:inline">Admin</span></Link>}<Link href="/account" aria-label="Switch to customer view" className="rounded-full border border-[#183126]/15 bg-[#faf9f5] px-3 py-2.5 text-xs font-bold transition hover:border-[#4d725d] hover:bg-[#dfead9] sm:px-4 sm:text-sm">↔ <span className="hidden min-[480px]:inline"><span className="hidden sm:inline">Switch to </span>customer</span></Link><NotificationBell /><ProfileAvatar name={accountName} imageUrl={session?.user.image} className="hidden h-10 w-10 text-sm min-[520px]:grid" /></div>
        </div>
      </header>
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-4 py-6 sm:px-8 sm:py-8 lg:grid-cols-[210px_1fr] lg:gap-8">
        ...
        <div className="min-w-0">
          <nav className="mb-6 flex snap-x gap-2 overflow-x-auto pb-2 text-sm font-bold [scrollbar-width:none] lg:hidden">{dashboardNav.map((item) => <Link key={item.section} href={item.href} className={`shrink-0 snap-start rounded-full px-4 py-2.5 ${section === item.section ? "bg-[#183126] text-white" : "border border-[#183126]/10 bg-white"}`}>{item.label}</Link>)}</nav>
          {notice && provider && <div className="mb-6 flex items-start justify-between gap-5 rounded-2xl border border-[#a8c1a9] bg-[#e8f2e7] p-4 text-sm"><div><p className="font-bold">Welcome to BubsBookings, {firstName}!</p><p className="mt-1 text-[#567060]">Your provider profile and services are saved.</p></div><button onClick={() => setNotice(false)} aria-label="Dismiss" className="rounded-full px-2 text-lg text-[#64786a] transition hover:bg-[#cbdcc8]">×</button></div>}
