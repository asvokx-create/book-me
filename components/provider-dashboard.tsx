...
type ProviderService = { id: string; companyId: string; companySlug: string; businessName: string; locationId: string; locationName: string; location: string; slug: string; title: string; category: string; price: number; durationMinutes: number; imageUrls: string[] };
    ...
    <main className="min-h-screen scroll-smooth bg-[#f4f4ef] text-[#183126]">
      <header className="sticky top-0 z-50 border-b border-[#183126]/10 bg-white/95 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-3 py-3 sm:px-8 sm:py-4">
          <Link href="/" aria-label="BubsBookings home" className="flex min-w-0 items-center gap-2 text-lg font-bold tracking-tight sm:text-xl"><BrandMark className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[#183126] text-sm text-[#eee25a]" /><span className="hidden sm:inline">BubsBookings</span><span className="hidden rounded-full bg-[#e8f0e5] px-2.5 py-1 text-[10px] uppercase tracking-wider text-[#55705e] sm:inline">Provider</span></Link>
