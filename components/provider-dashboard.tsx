...
type ProviderService = { id: string; businessName: string; slug: string; title: string; category: string; price: number; durationMinutes: number; imageUrls: string[] };
...
export default function ProviderDashboard({ section = "overview", initialConversationId = "" }: { section?: DashboardSection; initialConversationId?: string }) {
  ...
  const visibleNav = isWorker ? dashboardNav.filter((item) => ["overview", "bookings", "team", "settings"].includes(item.section)) : dashboardNav;
        ...
        <div className="min-w-0">
          ...
          <div className="mt-6 grid gap-5 xl:grid-cols-2">
            ...
            <section className="rounded-[2rem] border border-[#183126]/10 bg-white p-6">
              ...
              {provider?.services.length ? <div className="mt-5 space-y-3">{provider.services.slice(0, 3).map((service) => <div key={service.id} className="flex items-center gap-4 rounded-2xl bg-[#f5f5ef] p-4"><span role="img" aria-label={`${service.title} cover`} style={service.imageUrls[0] ? { backgroundImage: `url("${service.imageUrls[0]}")` } : undefined} className={`grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-cover bg-center ${service.imageUrls[0] ? "" : "bg-gradient-to-br from-lime-700 to-yellow-200 text-2xl"}`}>{service.imageUrls[0] ? "" : "🧰"}</span><div className="min-w-0 flex-1"><p className="truncate font-bold">{service.title}</p><p className="mt-1 truncate text-xs font-semibold text-[#52665b]">{service.businessName}</p><p className="mt-1 text-xs text-[#738179]">From ${service.price} · {formatDuration(service.durationMinutes)}</p></div><Link href={`/provider/services/${service.id}/edit`} className="rounded-full border border-[#183126]/15 bg-white px-3 py-2 text-xs font-bold">Edit</Link></div>)}</div> : <div className="mt-5 rounded-2xl bg-[#f5f5ef] px-5 py-8 text-center"><p className="text-2xl">◇</p><p className="mt-2 font-bold">No active listings</p><Link href="/providers/join" className="mt-3 inline-flex text-sm font-bold underline">Create your first service</Link></div>}
            </section>
          </div>
          ...
          {section === "services" && !isWorker && <div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
            <section className="rounded-[2rem] border border-[#183126]/10 bg-white p-6"><div className="flex items-center justify-between"><div><h2 className="text-xl font-bold">Your services</h2><p className="mt-1 text-xs text-[#738179]">Edit details, photos, pricing, or remove a listing.</p></div><Link href="/providers/join" className="text-sm font-bold">+ Add</Link></div>{provider?.services.length ? <div className="mt-5 space-y-5">{provider.services.map((service) => <div key={service.id} className="rounded-2xl bg-[#f5f5ef] p-4"><div className="flex items-center gap-4"><span role="img" aria-label={`${service.title} cover`} style={service.imageUrls[0] ? { backgroundImage: `url("${service.imageUrls[0]}")` } : undefined} className={`grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-cover bg-center ${service.imageUrls[0] ? "" : "bg-gradient-to-br from-lime-700 to-yellow-200 text-3xl"}`}>{service.imageUrls[0] ? "" : "🧰"}</span><div className="min-w-0 flex-1"><p className="truncate font-bold">{service.title}</p><p className="mt-1 truncate text-xs font-semibold text-[#52665b]">{service.businessName}</p><p className="mt-1 text-xs text-[#738179]">From ${service.price} · {formatDuration(service.durationMinutes)}</p></div><Link href={`/provider/services/${service.id}/edit`} className="rounded-full border border-[#183126]/15 bg-white px-4 py-2 text-xs font-bold hover:border-[#4d725d]">Edit</Link></div><ServiceImageManager serviceId={service.id} initialImageUrls={service.imageUrls} compact /></div>)}</div> : <p className="mt-5 rounded-2xl bg-[#f5f5ef] p-5 text-sm text-[#738179]">You do not have any active services. Add one to appear in customer searches.</p>}</section>
