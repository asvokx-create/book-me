...
export async function generateMetadata({ params }: PageProps<"/services/[slug]">): Promise<Metadata> {
  ...
  if (!service) return {};
  const stressTestService = process.env.NODE_ENV !== "production" && service.id.startsWith("stress-test-");
  ...
  return (
    <main className="min-h-screen bg-[#f8f7f3] text-[#183126]">
      ...
      <div className="mx-auto max-w-6xl px-4 py-7 sm:px-6 sm:py-12">
        <Link href={stressTestService ? "/services?stressTest=1#service-listings" : "/services"} className="inline-flex items-center gap-2 text-sm font-semibold text-[#5f7268] transition hover:text-[#183126]">← Back to services</Link>
        <div className="mt-7 grid gap-7 lg:grid-cols-[1.25fr_.75fr] lg:gap-10">
          <div>
            ...
            <div className="py-8">
              ...
              <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-3 text-sm"><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${service.city}, ${service.state}`)}`} target="_blank" rel="noreferrer" className="rounded-full px-2 py-1 text-[#718078] transition hover:bg-[#e5eddf] hover:text-[#183126]">📍 {service.city}, {service.state} · View map</a><span className="text-[#718078]">Offered by {stressTestService ? <strong className="text-[#183126]">{service.provider}</strong> : <Link href={service.companySlug ? `/companies/${service.companySlug}` : `/providers/${service.providerId}`} className="font-bold text-[#183126] underline decoration-[#c7bb41] decoration-2 underline-offset-4">{service.provider}</Link>}</span>{service.profileScreened && <span className="rounded-full bg-[#edf2e9] px-3 py-1 text-xs font-bold text-[#4f6d5a]">✓ Profile screened</span>}{service.emailVerified && <span className="rounded-full bg-[#e5f1e5] px-3 py-1 text-xs font-bold text-[#376447]">✓ Email verified</span>}{service.businessVerified && <span className="rounded-full bg-[#fff3b0] px-3 py-1 text-xs font-bold text-[#735f16]">✓ Business profile checked</span>}{!stressTestService && <ContactProviderLink providerId={service.providerId} serviceId={service.id} className="rounded-full border border-[#183126]/15 bg-white px-4 py-2 font-bold text-[#183126] transition hover:border-[#597563] hover:bg-[#e5eddf]" />}</div>
