        ...
        <div className="mt-7 grid gap-10 lg:grid-cols-[1.25fr_.75fr]">
          <div>
            ...
            <div className="py-8">
              ...
              <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-3 text-sm"><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${service.city}, ${service.state}`)}`} target="_blank" rel="noreferrer" className="rounded-full px-2 py-1 text-[#718078] transition hover:bg-[#e5eddf] hover:text-[#183126]">📍 {service.city}, {service.state} · View map</a><span className="text-[#718078]">Served by <Link href={`/providers/${service.providerId}`} className="font-bold text-[#183126] underline decoration-[#c7bb41] decoration-2 underline-offset-4">{service.provider}</Link></span>{service.profileScreened && <span className="rounded-full bg-[#edf2e9] px-3 py-1 text-xs font-bold text-[#4f6d5a]">✓ Profile screened</span>}{service.emailVerified && <span className="rounded-full bg-[#e5f1e5] px-3 py-1 text-xs font-bold text-[#376447]">✓ Email verified</span>}{service.businessVerified && <span className="rounded-full bg-[#fff3b0] px-3 py-1 text-xs font-bold text-[#735f16]">✓ Business verified</span>}<ContactProviderLink providerId={service.providerId} serviceId={service.id} className="rounded-full border border-[#183126]/15 bg-white px-4 py-2 font-bold text-[#183126] transition hover:border-[#597563] hover:bg-[#e5eddf]" /></div>
              <p className="mt-7 max-w-2xl text-lg leading-8 text-[#5b6d64]">{service.description}</p>
              <div className="mt-10 border-t border-[#183126]/10 pt-9"><h2 className="text-2xl font-bold tracking-tight">Service details</h2><div className="mt-6 grid gap-4 sm:grid-cols-2"><div className="rounded-2xl bg-white p-5 shadow-[0_4px_18px_rgba(24,49,38,.04)]"><p className="text-xs font-bold uppercase tracking-wider text-[#718078]">Typical duration</p><p className="mt-2 font-bold">{duration}</p></div><div className="rounded-2xl bg-white p-5 shadow-[0_4px_18px_rgba(24,49,38,.04)]"><p className="text-xs font-bold uppercase tracking-wider text-[#718078]">Starting price</p><p className="mt-2 font-bold">${service.price}</p></div></div><div className="mt-4 rounded-2xl border border-[#183126]/10 bg-white p-5"><p className="text-xs font-bold uppercase tracking-wider text-[#718078]">Cancellation policy</p><p className="mt-2 text-sm leading-6 text-[#5f7067]">{service.cancellationPolicy}</p><p className="mt-2 text-xs font-semibold text-[#718078]">Standard notice window: {service.cancellationWindowHours} hours</p></div></div>
            </div>
          </div>
          <aside><BookingCard serviceId={service.id} price={service.price} duration={duration} serviceTitle={service.title} provider={service.provider} isSignedIn={!isAuthConfigured() || Boolean(session)} returnPath={`/services/${service.slug}`} cancellationPolicy={service.cancellationPolicy} cancellationWindowHours={service.cancellationWindowHours} noShowPolicy={service.noShowPolicy} /></aside>
        </div>
      </div>
    </main>
  );
