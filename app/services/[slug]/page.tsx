        ...
        <div className="mt-7 grid gap-10 lg:grid-cols-[1.25fr_.75fr]">
          <div>
            ...
            <div className="py-8">
              ...
              <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-3 text-sm"><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${service.city}, ${service.state}`)}`} target="_blank" rel="noreferrer" className="rounded-full px-2 py-1 text-[#718078] transition hover:bg-[#e5eddf] hover:text-[#183126]">📍 {service.city}, {service.state} · View map</a><span className="text-[#718078]">Served by <Link href={`/providers/${service.providerId}`} className="font-bold text-[#183126] underline decoration-[#c7bb41] decoration-2 underline-offset-4">{service.provider}</Link></span>{service.emailVerified && <span className="rounded-full bg-[#e5f1e5] px-3 py-1 text-xs font-bold text-[#376447]">✓ Email verified</span>}{service.businessVerified && <span className="rounded-full bg-[#fff3b0] px-3 py-1 text-xs font-bold text-[#735f16]">✓ Business verified</span>}<ContactProviderLink providerId={service.providerId} serviceId={service.id} className="rounded-full border border-[#183126]/15 bg-white px-4 py-2 font-bold text-[#183126] transition hover:border-[#597563] hover:bg-[#e5eddf]" /></div>
