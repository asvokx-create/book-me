...
export default async function AffiliateDashboardPage({ searchParams }: { searchParams: Promise<{ range?: string }> }) {
  ...
  if (!profile.affiliate_code || !["approved", "active", "paused"].includes(profile.status)) return <main className="min-h-screen bg-[#f8f7f3] text-[#183126]"><div className="reading-container px-5 py-16"><Link href="/"><BrandLockup /></Link><div className="mt-10 rounded-[2rem] bg-white p-8"><p className="text-xs font-bold uppercase tracking-wider text-[#687970]">Application status</p><h1 className="mt-2 text-3xl font-bold capitalize">{profile.status.replace("_", " ")}</h1><p className="mt-3 leading-7 text-[#687970]">Your application is not active yet. BubsBookings will review it before issuing a referral code.</p></div></div></main>;

  const [statsResult, referralsResult, commissionsResult, payoutsResult] = await Promise.all([
    database.query<{ clicks: number; signups: number; qualified: number; bookings: number; eligible_revenue: number; pending: number; payable: number; paid: number }>(`SELECT
      ...
      (SELECT COALESCE(sum(amount_cents),0)::int FROM affiliate_commissions WHERE affiliate_id = $1 AND status = 'paid' AND ($2::text IS NULL OR created_at >= CASE WHEN $2='year' THEN date_trunc('year',now()) ELSE now()-$2::interval END)) AS paid`, [profile.id, since]),
  ...
  return <main className="min-h-screen bg-[#f8f7f3] text-[#183126]"><header className="border-b border-[#183126]/10 bg-white"><div className="dashboard-container flex items-center justify-between px-5 py-4"><Link href="/"><BrandLockup /></Link><Link href="/account" className="rounded-full border border-[#183126]/15 px-5 py-2.5 text-sm font-bold">Account</Link></div></header>
    ...
    <section className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[["Clicks", stats.clicks], ["Provider signups", stats.signups], ["Qualified providers", stats.qualified], ["Signup conversion", `${conversion}%`], ["Bookings generated", stats.bookings], ["Eligible platform revenue", money(stats.eligible_revenue)], ["Pending commissions", money(stats.pending)], ["Payable balance", money(stats.payable)]].map(([label,value]) => <article key={label} className="rounded-2xl border border-[#183126]/10 bg-white p-5"><p className="text-xs font-bold uppercase tracking-wider text-[#718078]">{label}</p><p className="mt-2 text-2xl font-bold">{value}</p></article>)}</section>
    <div className="mt-7"><AffiliateLinkBuilder code={profile.affiliate_code} /></div>
