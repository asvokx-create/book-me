...
export default async function CompanyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="min-h-screen bg-[#f5f4ef] text-[#183126]">
    <header className="sticky top-0 z-50 border-b border-[#183126]/10 bg-white/95 backdrop-blur-xl"><div className="mx-auto flex max-w-6xl items-center justify-between gap-2 px-4 py-4 sm:px-8"><Link href="/" className="flex min-w-0 items-center gap-2.5 text-xl font-bold"><BrandLockup compact /></Link><div className="flex items-center gap-2"><MobileSiteNav /><AccountNav /></div></div></header>
    <section className="bg-[#183126] text-white"><div className="mx-auto max-w-6xl px-5 py-12 sm:px-8 sm:py-16"><BackButton label="Back to services" fallbackHref="/services" /><div className="mt-7 flex flex-col justify-between gap-6 sm:flex-row sm:items-end"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-[#eee25a]">Company page</p><h1 className="mt-3 text-4xl font-bold tracking-[-.05em] sm:text-5xl">{company.name}</h1><p className="mt-4 text-[#c1d0c8]">Owned by {company.ownerName} · Serving {company.locations.length} {company.locations.length === 1 ? "location" : "locations"}</p></div>{company.verified && <span className="w-fit rounded-full bg-[#e4f1e5] px-4 py-2 text-xs font-bold text-[#34704a]">✓ BubsBookings screened</span>}</div>{company.bio && <p className="mt-7 max-w-3xl text-base leading-7 text-[#d2ddd7]">{company.bio}</p>}<div className="mt-6 flex flex-wrap gap-2">{company.locations.map((location) => { const cityState = `${location.city}, ${location.state}`; const locationLabel = location.name.trim().toLowerCase() === cityState.toLowerCase() ? cityState : `${location.name} · ${cityState}`; return <span key={location.id} className="rounded-full bg-white/10 px-3 py-2 text-xs font-semibold">⌖ {locationLabel} · {location.serviceRadiusMiles} mi</span>; })}</div></div></section>
