...
export default async function OperationsPage() {
  ...
  const [emails, checks, activity, funnel, refunds, unmetDemand, zeroResultDemand, providerFunnel, marketplaceSupply, marketplaceEconomics, supplyByCity, supplyByZip, supplyByCategory, demandByCity, demandByCategory] = await Promise.all([
    ...
    database.query<{ location: string; category: string; search_term: string; radius_miles: number; searches: number; latest_search: Date; current_providers: number }>(
      `WITH gaps AS (
       ...
       SELECT gaps.*,
         (SELECT count(DISTINCT service.provider_id)::int
          FROM services service
          JOIN provider_profiles provider ON provider.id = service.provider_id AND provider.is_active = true
          JOIN provider_locations location ON location.id = service.location_id AND location.is_active = true
          WHERE service.is_active = true
            AND (gaps.category = 'All services' OR lower(service.category) = lower(gaps.category))
            AND (lower(location.city || ', ' || location.state) = lower(gaps.location)
              OR EXISTS (SELECT 1 FROM provider_location_service_areas area WHERE area.location_id = location.id
                AND (lower(area.city || ', ' || area.state) = lower(gaps.location) OR area.postal_code = gaps.location)))) AS current_providers
       FROM gaps ORDER BY searches DESC, latest_search DESC LIMIT 20`,
    ),
    ...
    database.query<{ city: string; state: string; active_providers: number; active_listings: number }>(
      `WITH coverage AS (
         SELECT service.provider_id, service.id AS service_id, location.city, location.state
         FROM services service JOIN provider_profiles provider ON provider.id = service.provider_id AND provider.is_active = true
         JOIN provider_locations location ON location.id = service.location_id AND location.is_active = true
         WHERE service.is_active = true
         UNION
         SELECT service.provider_id, service.id, area.city, area.state
         FROM services service JOIN provider_profiles provider ON provider.id = service.provider_id AND provider.is_active = true
         JOIN provider_locations location ON location.id = service.location_id AND location.is_active = true
         JOIN provider_location_service_areas area ON area.location_id = location.id
         WHERE service.is_active = true
       ) SELECT city, state, count(DISTINCT provider_id)::int AS active_providers,
           count(DISTINCT service_id)::int AS active_listings
         FROM coverage GROUP BY city, state ORDER BY active_providers DESC, active_listings DESC, city LIMIT 20`,
    ),
    database.query<{ postal_code: string; city: string; state: string; active_providers: number; active_listings: number }>(
      `SELECT area.postal_code, area.city, area.state, count(DISTINCT service.provider_id)::int AS active_providers,
              count(DISTINCT service.id)::int AS active_listings
       FROM provider_location_service_areas area
       JOIN provider_locations location ON location.id = area.location_id AND location.is_active = true
       JOIN services service ON service.location_id = location.id AND service.is_active = true
       JOIN provider_profiles provider ON provider.id = service.provider_id AND provider.is_active = true
       WHERE area.area_type = 'zip' AND area.postal_code IS NOT NULL
       GROUP BY area.postal_code, area.city, area.state
       ORDER BY active_providers DESC, active_listings DESC, area.postal_code LIMIT 20`,
    ),
    database.query<{ category: string; active_providers: number; active_listings: number }>(
      `SELECT service.category, count(DISTINCT service.provider_id)::int AS active_providers,
              count(service.id)::int AS active_listings
       FROM services service JOIN provider_profiles provider ON provider.id = service.provider_id
       WHERE service.is_active = true AND provider.is_active = true
       GROUP BY service.category ORDER BY active_providers DESC, active_listings DESC, service.category`,
    ),
    database.query<{ location: string; demand: number }>(
      `SELECT location, count(*)::int AS demand FROM service_demand_requests
       WHERE created_at >= now() - interval '30 days' GROUP BY location ORDER BY demand DESC, location LIMIT 12`,
    ),
    database.query<{ category: string; demand: number }>(
      `SELECT COALESCE(category, 'Any service') AS category, count(*)::int AS demand FROM service_demand_requests
       WHERE created_at >= now() - interval '30 days' GROUP BY category ORDER BY demand DESC, category LIMIT 12`,
    ),
  ]);
  ...
  return <main className="min-h-screen overflow-x-clip bg-[#f4f4ef] px-4 py-7 text-[#183126] sm:px-8 sm:py-8"><div className="mx-auto min-w-0 max-w-5xl">
    <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#718078]">Admin operations</p><h1 className="mt-2 text-3xl font-bold">Reliability center</h1></div><BackButton label="Back to admin" fallbackHref="/admin" /></div>
    ...
    <section className="mt-6 rounded-[2rem] border border-[#183126]/10 bg-white p-6"><h2 className="text-xl font-bold">30-day marketplace conversion</h2><p className="mt-1 text-sm text-[#718078]">Use the rates—not feature count—to find the largest customer and provider drop-offs.</p><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><MiniMetric label="Searches with results" value={events.search_results ?? 0} /><MiniMetric label="Zero-result searches" value={events.zero_result_search ?? 0} /><MiniMetric label="Listing views" value={events.service_view ?? 0} detail={`${searchToViewRate}% of searches`} /><MiniMetric label="Provider views" value={events.provider_profile_view ?? 0} /><MiniMetric label="Conversations started" value={events.message_started ?? 0} /><MiniMetric label="Messages" value={events.message_sent ?? 0} detail={`${viewToMessageRate}% of views`} /><MiniMetric label="Quotes sent" value={events.job_quote_sent ?? 0} /><MiniMetric label="Quotes accepted" value={events.job_quote_accepted ?? 0} /><MiniMetric label="Booking requests" value={events.booking_requested ?? 0} /><MiniMetric label="Bookings confirmed" value={events.booking_confirmed ?? 0} /><MiniMetric label="Checkout starts" value={events.checkout_started ?? 0} /><MiniMetric label="Payments" value={events.payment_completed ?? 0} detail={`${bookingToPaymentRate}% of requests`} /><MiniMetric label="Completed bookings" value={events.booking_completed ?? 0} /><MiniMetric label="Reviews" value={events.review_submitted ?? 0} /><MiniMetric label="Rebookings" value={events.customer_rebooked ?? 0} /><MiniMetric label="Disputes" value={events.booking_disputed ?? 0} /></div><div className="mt-4 rounded-2xl bg-[#f5f5ef] p-4 text-sm"><strong>Refund queue:</strong> {refundTotals.requested ?? 0} awaiting review · {refundTotals.refunded ?? 0} completed</div></section>
    <section className="mt-6 rounded-[2rem] border border-[#183126]/10 bg-white p-6"><h2 className="text-xl font-bold">30-day provider activation</h2><p className="mt-1 text-sm text-[#718078]">From provider profile creation to first listing, quote, booking, and completion.</p><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><MiniMetric label="Provider signups" value={providerMetrics.signups} /><MiniMetric label="Published a listing" value={providerMetrics.first_listings} detail={`${signupToListingRate}% of signups`} /><MiniMetric label="First quotes sent" value={events.first_quote_sent ?? 0} /><MiniMetric label="Received a booking" value={providerMetrics.first_bookings} detail={`${signupToBookingRate}% of signups`} /><MiniMetric label="First bookings completed" value={events.first_booking_completed ?? 0} /><MiniMetric label="Inactive providers" value={economics.inactive_providers} detail="No booking in 60 days" /></div><p className="mt-4 text-sm text-[#617169]">Average time to first listing: <strong>{formatHours(providerMetrics.avg_hours_to_listing)}</strong> · Average time to first booking: <strong>{formatHours(providerMetrics.avg_hours_to_booking)}</strong></p></section>
    ...
    <section className="mt-6 rounded-[2rem] border border-[#183126]/10 bg-white p-5 sm:p-6"><h2 className="text-xl font-bold">Supply recruiting overview</h2><p className="mt-1 text-sm text-[#718078]">Active supply uses published listings and provider-defined service coverage. Markets below five providers are recruiting priorities, not customer-ready claims.</p><div className="mt-5 grid gap-5 lg:grid-cols-3"><Leaderboard title="Providers by city" empty="No active city coverage yet." rows={supplyByCity.rows.map((row) => ({ label: `${row.city}, ${row.state}`, value: row.active_providers, detail: `${row.active_listings} listings`, low: row.active_providers < 5 }))} /><Leaderboard title="Providers by ZIP" empty="No ZIP-specific coverage yet." rows={supplyByZip.rows.map((row) => ({ label: `${row.postal_code} · ${row.city}, ${row.state}`, value: row.active_providers, detail: `${row.active_listings} listings`, low: row.active_providers < 5 }))} /><Leaderboard title="Providers by category" empty="No active categories yet." rows={supplyByCategory.rows.map((row) => ({ label: row.category, value: row.active_providers, detail: `${row.active_listings} listings`, low: row.active_providers < 5 }))} /></div></section>
    <section className="mt-6 rounded-[2rem] border border-[#183126]/10 bg-white p-5 sm:p-6"><h2 className="text-xl font-bold">30-day customer demand</h2><p className="mt-1 text-sm text-[#718078]">Aggregated, consented unmet-demand signals. No customer names or contact details are shown.</p><div className="mt-5 grid gap-5 md:grid-cols-2"><Leaderboard title="Demand by city" empty="No city demand captured yet." rows={demandByCity.rows.map((row) => ({ label: row.location, value: row.demand, detail: "requests" }))} /><Leaderboard title="Demand by category" empty="No category demand captured yet." rows={demandByCategory.rows.map((row) => ({ label: row.category, value: row.demand, detail: "requests" }))} /></div></section>
    ...
    <section className="mt-6 rounded-[2rem] border border-[#183126]/10 bg-white p-5 sm:p-6"><h2 className="text-xl font-bold">Zero-result search gaps</h2><p className="mt-1 text-sm text-[#718078]">Every search that returned no listings, grouped by city, radius, category, and search term for the last 30 days. “Supply now” shows whether active providers have since entered that exact city or ZIP/category.</p><div className="admin-table-scroll mt-5 max-w-full overflow-x-auto overscroll-x-contain rounded-xl" role="region" aria-label="Zero-result search gaps table" tabIndex={0}><table className="w-full min-w-[820px] text-left text-sm"><thead><tr className="border-b border-[#183126]/15 text-xs uppercase tracking-wider text-[#718078]"><th className="px-3 py-3">Market</th><th className="px-3 py-3">Category</th><th className="px-3 py-3">Search term</th><th className="px-3 py-3">Radius</th><th className="px-3 py-3 text-right">Searches</th><th className="px-3 py-3">Supply now</th></tr></thead><tbody className="divide-y divide-[#183126]/10">{zeroResultDemand.rows.map((row) => <tr key={`${row.location}-${row.category}-${row.search_term}-${row.radius_miles}`}><td className="px-3 py-3 font-bold">{row.location}</td><td className="px-3 py-3">{row.category}</td><td className="px-3 py-3">{row.search_term}</td><td className="px-3 py-3">{row.radius_miles} mi</td><td className="px-3 py-3 text-right font-bold">{row.searches}</td><td className="px-3 py-3"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${row.current_providers > 0 ? "bg-[#e4f1e5] text-[#35704a]" : "bg-[#fff3c7] text-[#78681f]"}`}>{row.current_providers > 0 ? `${row.current_providers} providers` : "Still no supply"}</span></td></tr>)}{zeroResultDemand.rows.length === 0 && <tr><td colSpan={6} className="px-3 py-8 text-center text-[#718078]">No zero-result searches have been recorded in the last 30 days.</td></tr>}</tbody></table></div></section>
    <section className="mt-6 rounded-[2rem] border border-[#183126]/10 bg-white p-5 sm:p-6"><h2 className="text-xl font-bold">Supply density by market</h2><p className="mt-1 text-sm text-[#718078]">Concentrate recruiting in one metro until core categories offer real customer choice. Aim for 5–10 useful providers in each priority category.</p><div className="admin-table-scroll mt-5 max-w-full overflow-x-auto overscroll-x-contain rounded-xl" role="region" aria-label="Supply density by market table" tabIndex={0}><table className="w-full min-w-[620px] text-left text-sm"><thead><tr className="border-b border-[#183126]/15 text-xs uppercase tracking-wider text-[#718078]"><th className="px-3 py-3">Market</th><th className="px-3 py-3">Category</th><th className="px-3 py-3 text-right">Providers</th><th className="px-3 py-3 text-right">Listings</th><th className="px-3 py-3">Density</th></tr></thead><tbody className="divide-y divide-[#183126]/10">{marketplaceSupply.rows.map((row) => <tr key={`${row.city}-${row.state}-${row.category}`}><td className="px-3 py-3 font-bold">{row.city}, {row.state}</td><td className="px-3 py-3">{row.category}</td><td className="px-3 py-3 text-right">{row.active_providers}</td><td className="px-3 py-3 text-right">{row.active_listings}</td><td className="px-3 py-3"><span className={`rounded-full px-3 py-1 text-xs font-bold ${row.active_providers >= 5 ? "bg-[#e4f1e5] text-[#35704a]" : "bg-[#fff3c7] text-[#78681f]"}`}>{row.active_providers >= 5 ? "Ready for demand" : `Recruit ${5 - row.active_providers}+`}</span></td></tr>)}{marketplaceSupply.rows.length === 0 && <tr><td colSpan={5} className="px-3 py-8 text-center text-[#718078]">No active supply is available yet.</td></tr>}</tbody></table></div></section>
...
function Leaderboard({ title, rows, empty }: { title: string; rows: Array<{ label: string; value: number; detail: string; low?: boolean }>; empty: string }) {
  return <div className="min-w-0 rounded-2xl bg-[#f5f5ef] p-4"><h3 className="font-bold">{title}</h3><div className="mt-3 divide-y divide-[#183126]/10">{rows.slice(0, 8).map((row) => <div key={row.label} className="flex min-w-0 items-center justify-between gap-3 py-3"><div className="min-w-0"><p className="break-words text-sm font-bold">{row.label}</p><p className="mt-0.5 text-xs text-[#718078]">{row.detail}</p></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-bold ${row.low ? "bg-[#fff3c7] text-[#78681f]" : "bg-white text-[#183126]"}`}>{row.value}</span></div>)}{rows.length === 0 && <p className="py-5 text-sm text-[#718078]">{empty}</p>}</div></div>;
}
