    ...
    <div className="rounded-[2rem] border border-[#183126]/10 bg-white p-6 shadow-[0_24px_60px_rgba(24,49,38,.12)] sm:p-9">
      ...
      <form onSubmit={next} className="mt-8">
        {step === 1 && <div className="space-y-5">
          ...
          <label className="block"><span className="mb-2 block text-sm font-bold">Service area</span><select value={city} onChange={(event) => setCity(event.target.value)} className={inputClass}>{SERVICE_AREAS.map((area) => { const label = serviceAreaLabel(area); return <option key={label} value={label}>{label}</option>; })}</select><span className="mt-2 block text-xs text-[#74827b]">BookMe uses this verified city coordinate to calculate customer distance and radius results.</span></label>
        </div>}
