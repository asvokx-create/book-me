    ...
    <main className="min-h-screen bg-[#f8f7f3] text-[#183126]">
      ...
      <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 sm:py-12">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1.15fr_.85fr]">
          ...
          <aside>{featured ? <section className="rounded-[2rem] border border-[#183126]/10 bg-white p-6 shadow-[0_10px_30px_rgba(24,49,38,.07)]"><p className="text-xs font-bold uppercase tracking-[.14em] text-[#708078]">Featured service</p><div className={`mt-4 grid h-36 place-items-center rounded-2xl bg-gradient-to-br ${visual.gradient} text-6xl`}>{visual.art}</div><h2 className="mt-5 text-xl font-bold">{featured.title}</h2><p className="mt-2 text-sm text-[#718078]">{formatDuration(featured.durationMinutes)}</p><div className="mt-5 flex items-center justify-between border-t border-[#183126]/10 pt-5"><p className="font-bold">From ${featured.price}</p><Link href={`/services/${featured.slug}`} className="rounded-full bg-[#183126] px-5 py-2.5 text-sm font-bold text-white">View service</Link></div></section> : <section className="rounded-[2rem] border border-[#183126]/10 bg-white p-6"><p className="font-bold">No active services</p></section>}</aside>
        </div>
      </div>
    </main>
  );
