    ...
    <main className="min-h-screen bg-[#f5f4ef] px-5 py-8 text-[#183126] sm:px-8">
      <div className="mx-auto max-w-3xl">
        <div className="flex items-center justify-between gap-4">
          ...
          <BackButton label="Back to my account" fallbackHref="/account" />
        </div>
        ...
        <section className="mt-10 rounded-[2rem] border border-[#183126]/10 bg-white p-6 shadow-[0_18px_55px_rgba(24,49,38,.08)] sm:p-9">
          ...
          {twoFactorEnabled && <form onSubmit={disableTwoFactor} className="mt-7 border-t border-[#183126]/10 pt-6"><h2 className="font-bold">Turn off authenticator</h2><p className="mt-1 text-sm text-[#6f7f77]">You will need your current password.</p><label htmlFor="disable-two-factor-password" className="sr-only">Current password</label><input id="disable-two-factor-password" required type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} className={`${inputClass} mt-4`} /><button disabled={loading} className="mt-3 rounded-full px-4 py-2 text-sm font-bold text-[#914e3a] transition hover:bg-[#f4d8cc]">{loading ? "Please wait…" : "Turn off authenticator"}</button></form>}
        </section>
      </div>
    </main>
  );
