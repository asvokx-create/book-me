...
export default async function CustomerMessagesPage({ searchParams }: { searchParams: Promise<{ conversationId?: string; providerId?: string; serviceId?: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[#f5f4ef] text-[#183126]">
      <header className="relative z-50 border-b border-[#183126]/10 bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-4 py-4 sm:px-8"><Link href="/" className="flex min-w-0 items-center gap-2 text-lg font-bold tracking-tight sm:gap-2.5 sm:text-xl"><BrandMark className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[#183126] text-sm text-[#eee25a]" /><span className="hidden min-[390px]:inline">BubsBookings</span><span className="min-[390px]:hidden">Bubs</span></Link><AccountNav /></div></header>
