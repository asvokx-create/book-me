...
export default async function CustomerMessagesPage({ searchParams }: { searchParams: Promise<{ conversationId?: string; providerId?: string; serviceId?: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[#f5f4ef] text-[#183126]">
      <header className="relative z-50 border-b border-[#183126]/10 bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-4 py-4 sm:px-8"><Link href="/" className="flex min-w-0 items-center gap-2 text-lg font-bold tracking-tight sm:gap-2.5 sm:text-xl"><BrandLockup /></Link><AccountNav /></div></header>
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-8 sm:py-12">
        <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><BackButton label="Back to my account" fallbackHref="/account" /><h1 className="mt-3 text-4xl font-bold tracking-[-.045em]">Your messages</h1><p className="mt-2 text-[#6c7c74]">Talk directly with local providers before and after booking.</p></div><Link href="/services" className="self-start rounded-full bg-[#eee25a] px-5 py-3 text-sm font-bold transition hover:bg-[#e1d43d] sm:self-auto">Find a provider</Link></div>
        <MessagingCenter
          mode="customer"
