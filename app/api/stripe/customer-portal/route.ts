...
export async function POST(request: Request) {
  ...
  const result = await database.query<{ stripe_customer_id: string | null }>(
    "SELECT stripe_customer_id FROM provider_profiles WHERE user_id = $1 AND is_active = true", [session.user.id],
  );
