...
export async function PATCH(request: Request) {
  ...
  if (!Number.isInteger(hours) || hours < 0 || hours > 168 || ![5, 10, 15, 25, 50, 100].includes(serviceRadiusMiles) || policy.length < 10 || policy.length > 500 || noShowPolicy.length < 10 || noShowPolicy.length > 500) return NextResponse.json({ error: "Choose a valid working radius and notice window, and keep both policies between 10 and 500 characters." }, { status: 400 });
