...
import { sendBookingReminder, type BookingEmailRow } from "@/lib/booking-email";
...
export async function POST(request: Request) {
  ...
  if (!isEmailConfigured()) return NextResponse.json({ error: "Transactional email is not configured." }, { status: 503 });
