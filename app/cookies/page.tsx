...
const sections = [
  ...
  ["2. Necessary and preference storage", "BubsBookings uses session and security cookies or storage to keep users signed in, protect requests, complete Google sign-in, and provide account and marketplace features. Browser storage also remembers choices such as theme, time zone, selected service area and radius, whether the location prompt was shown, and a temporary post-login destination. Blocking or deleting necessary storage can sign you out, reset preferences, or prevent account features from working."],
  ["3. Analytics", <><p>BubsBookings stores a random first-party analytics identifier in local storage and records product events such as searches, zero-result searches, page and listing views, service requests and quotes, messages, booking and checkout steps, cancellations, and refunds. The site also uses Google Analytics 4 to measure traffic and feature use. These tools can use identifiers, device and browser information, IP-derived information, pages visited, and event timestamps.</p><p className="mt-3">Google explains <a href="https://policies.google.com/technologies/partner-sites" target="_blank" rel="noreferrer" className="font-bold underline">how it uses information from sites that use its services</a> and provides a <a href="https://tools.google.com/dlpage/gaoptout" target="_blank" rel="noreferrer" className="font-bold underline">Google Analytics opt-out browser add-on</a>.</p></>],
  ...
  ["5. Your controls", "You can use browser settings to delete or block cookies and local storage, and browser privacy tools or content blockers may limit analytics. Controls vary by browser and device. Disabling storage may affect sign-in, saved preferences, location selection, or other features. BubsBookings does not currently sell personal information or use it for cross-context behavioral advertising, so there is no sale or targeted-advertising activity to opt out of. A browser's Do Not Track signal does not have one universally accepted meaning; BubsBookings will honor legally required universal opt-out signals if a covered use is introduced."],
