...
const sections = [
  ...
  ["5. Bookings, payments, and fees", "A request is not confirmed until the provider accepts it. When online payment is available, Stripe processes the customer’s card and routes the provider share to the provider’s connected account. BubsBookings may keep the booking fee shown for the provider’s plan. Providers remain responsible for taxes, lawful pricing, fulfillment, and their connected payout account."],
  ["6. Cancellations and refunds", "Either participant may cancel as allowed in the booking flow and must provide a truthful reason. The provider’s displayed notice window determines whether a customer cancellation is marked late. Refunds return to the original payment method after the provider or BubsBookings approves them. Timing is controlled by Stripe and the customer’s bank. A cancellation does not automatically guarantee a refund; users may open a dispute for unresolved service or billing problems."],
