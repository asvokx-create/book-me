...
export default async function ProviderBookingPage({ params }: PageProps<"/provider/dashboard/bookings/[bookingId]">) {
  ...
  return <main className="min-h-screen bg-[#f5f4ef] text-[#183126]">
    <header className="border-b border-[#183126]/10 bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-8"><Link href="/" className="flex items-center gap-2.5 text-xl font-bold tracking-tight"><BrandMark className="grid h-9 w-9 place-items-center rounded-xl bg-[#183126] text-sm text-[#eee25a]" />BubsBookings</Link><Link href="/account" className="rounded-full border border-[#183126]/15 px-4 py-2 text-sm font-bold transition hover:bg-[#e7eee2]">Switch to customer</Link></div></header>
