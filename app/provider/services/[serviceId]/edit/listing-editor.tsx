        ...
        <div className="mt-6 rounded-[2rem] border border-[#183126]/10 bg-white p-6 shadow-[0_18px_50px_rgba(24,49,38,.08)] sm:p-9">
          ...
          {listing && <form onSubmit={save} className="mt-8 space-y-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <label className="block"><span className="mb-2 block text-sm font-bold">Estimated job duration</span><select value={durations.some((duration) => duration.value === listing.durationMinutes) ? String(listing.durationMinutes) : "__custom__"} onChange={(event) => change("durationMinutes", event.target.value === "__custom__" ? 1440 : Number(event.target.value))} className={inputClass}>{durations.map((duration) => <option key={duration.value} value={duration.value}>{duration.label}</option>)}<option value="__custom__">Custom duration…</option></select>{!durations.some((duration) => duration.value === listing.durationMinutes) && <div className="mt-3"><input type="number" min="0.25" max="168" step="0.25" value={listing.durationMinutes / 60} onChange={(event) => change("durationMinutes", Math.round(Number(event.target.value) * 60))} className={inputClass} /><span className="mt-2 block text-xs text-[#75837c]">Hours, including multi-day jobs (up to 168 hours).</span></div>}<span className="mt-2 block text-xs font-normal leading-5 text-[#75837c]">Used to reserve calendar availability. The actual job may finish sooner or later.</span></label>
            </div>
            <label className="block"><span className="mb-2 block text-sm font-bold">Service location</span><select value={listing.locationId} disabled={!listing.multipleLocationsAllowed} onChange={(event) => { const selected = listing.locations.find((item) => item.id === event.target.value); change("locationId", event.target.value); if (selected) { change("location", selected.location); change("locationName", selected.name); } }} className={`${inputClass} disabled:cursor-not-allowed disabled:bg-[#eef1eb]`}>{listing.locations.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.location}</option>)}</select><span className="mt-2 block text-xs text-[#849189]">{listing.multipleLocationsAllowed ? <>Manage branches and worker coverage from <Link href="/provider/dashboard/locations" className="font-bold underline">Service locations</Link>.</> : "This listing uses your primary location. Pro unlocks multiple locations."}</span></label>
