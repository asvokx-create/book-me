        ...
        <div className="mt-6 rounded-[2rem] border border-[#183126]/10 bg-white p-6 shadow-[0_18px_50px_rgba(24,49,38,.08)] sm:p-9">
          ...
          {listing && <form onSubmit={save} className="mt-8 space-y-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <label className="block"><span className="mb-2 block text-sm font-bold">Estimated job duration</span><select value={listing.durationMinutes} onChange={(event) => change("durationMinutes", Number(event.target.value))} className={inputClass}>{durations.map((duration) => <option key={duration.value} value={duration.value}>{duration.label}</option>)}</select><span className="mt-2 block text-xs font-normal leading-5 text-[#75837c]">Used to reserve calendar availability. The actual job may finish sooner or later.</span></label>
            </div>
